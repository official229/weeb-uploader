<script lang="ts">
	...
	const foldersAtFileDepth = $derived.by(() => {
		...
		return allFolders.filter((folderAtDepth) => {
			return folderAtDepth.files.some((file) => file.path.toLowerCase().includes(filterLower));
		});
	});
	...
	const groupedByName = $derived.by(() => {
		const groups = new Map<
			SelectedFolder,
			Array<{ file: SelectedFolder['files'][0]; folder: SelectedFolder }>
		>();
...
<div class="space-y-6">
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
		...
		<div class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
			<label
				for="name-depth-select"
				class="block font-medium text-gray-900 dark:text-gray-100 mb-2"
			>
					...
					>
						...
						<div class="flex-1">
							<div class="font-medium text-gray-900 dark:text-gray-100">
								{getFolderPath(folder, folderAtDepth) ?? folderAtDepth.name}
							</div>
							<div class="text-sm text-gray-600 dark:text-gray-400">
								{folderAtDepth.files.length} image{folderAtDepth.files.length !== 1 ? 's' : ''}
								{#if folderAtDepth.folders.length > 0}
									• {folderAtDepth.folders.length} subfolder{folderAtDepth.folders.length !== 1
										? 's'
										: ''}
								{/if}
							</div>
				...
				{#each foldersAtNameDepth as folderAtDepth}
					...
					>
						...
						<div class="flex-1">
							...
							<div class="text-sm text-gray-600 dark:text-gray-400">
								Parent of {selectedFileFolders.size > 0
									? Array.from(selectedFileFolders).filter((f) => {
											const parentMap = findParentsAtDepth(folder, [f], nameDepth);
											return parentMap.get(f) === folderAtDepth;
										}).length
									: 0} selected file folder{selectedFileFolders.size !== 1 ? 's' : ''}
							</div>
						</div>
					</div>
				{/each}
				...
				{#each groupedByName as group}
					<div class="p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
						<h4 class="font-semibold text-gray-900 dark:text-gray-100 mb-2">
							{getFolderPath(folder, group.nameFolder) ?? group.name} ({group.files.length} file{group
								.files.length !== 1
								? 's'
								: ''})
						</h4>
						<div class="space-y-1 max-h-48 overflow-y-auto">
							{#each group.files as { file, folder: fileFolder }}
								<div class="flex items-center gap-2 p-2 bg-white dark:bg-gray-700 rounded text-sm">
									...
									<span class="text-xs text-gray-500 dark:text-gray-400">
										from {getFolderPath(folder, fileFolder) ?? fileFolder.name}
									</span>
								</div>
							{/each}
						</div>
