	...
	async function applyTitlesToChapters(): Promise<number> {
		...
		for (let i = 0; i < targetingState.chapterStates.length; i++) {
			...
			if (foundMatch) {
				...
				// assign all groups and mark as a warning
				if (matchedGroupName) {
					...
					if (allGroupsInChapter.length > 1) {
						...
						// Track this as a partial match warning
						partialMatches.push({
							chapter,
				...
				>
					...
					<div class="flex flex-col gap-1 text-xs text-yellow-700 dark:text-yellow-300">
						...
						{#if partialGroupMatches.length > 0}
							<p>
								• {partialGroupMatches.length} chapter{partialGroupMatches.length === 1 ? '' : 's'} with
