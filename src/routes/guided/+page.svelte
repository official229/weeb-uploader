...
<div class="container mx-auto p-6 flex flex-col gap-6">
	...
	{#if guidedState.workflowStep === GuidedWorkflowStep.SELECTING_ROOT_FOLDER}
		<GuidedFolderSelector {guidedState} onDone={onFolderSelectionDone} class="w-full max-w-md mx-auto" />
	{:else if guidedState.workflowStep === GuidedWorkflowStep.SELECTING_MANGA_FOLDERS}
		<MangaFolderSelector {guidedState} onDone={onMangaSelectionDone} class="w-full max-w-4xl mx-auto" />
	{:else if guidedState.workflowStep === GuidedWorkflowStep.PROCESSING_MANGA}
		<div class="flex flex-col gap-4">
			<MangaProcessingTabs {guidedState} onZipSelected={onZipSelected} class="w-full" />
			...
			{:else}
				<div class="bg-surface rounded-md p-4">
					<p class="text-muted">No zip file selected. Use the tabs above to select a zip file to process.</p>
				</div>
			{/if}
		</div>
	{/if}
